...
// 1. Fungsi Anti-XSS (Sanitasi HTML)
...
// 2. Fungsi Bantuan Render Grid Container
...
function renderSuggestions(animes) {
    ...
    suggestionBox.innerHTML = animes.map(anime => `
        <div onclick="executeSearch('${escapeHTML(anime.title)}', true)" class="flex items-center gap-4 p-4 hover:bg-blue-600/20 cursor-pointer border-b border-white/5 last:border-none transition-all">
...
function renderGrid(animes) {
    ...
    container.innerHTML = animes.map(anime => `
        <div onclick="showDetail(${anime.mal_id}, true)" class="anime-card group bg-slate-800/40 rounded-3xl overflow-hidden border border-white/5 transition-all duration-500 cursor-pointer fade-in">
...
function fetchRandomAnime() { fetch('https://api.jikan.moe/v4/random/anime').then(r => r.json()).then(r => showDetail(r.data.mal_id, true)); }
...
function errorMsg() { app.innerHTML = `<div class="text-center py-32"><p class="text-red-500 font-black text-xl uppercase tracking-widest">Network Outage. Please Reload.</p></div>`; }
...
document.addEventListener('click', (e) => { if (!e.target.closest('#searchInput') && !e.target.closest('#suggestionBox')) suggestionBox.classList.add('hidden'); });
